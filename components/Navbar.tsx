...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] bg-navy-deep/95 backdrop-blur-xl border-b border-white/10 shadow-2xl transition-all duration-500">
      ...
      {/* Mobile Menu Overlay */}
      {isOpen && (
        <div className="lg:hidden absolute top-24 left-0 right-0 bg-navy-deep border-b border-white/10 animate-in slide-in-from-top-10 duration-500 shadow-2xl">
          <div className="flex flex-col p-8 gap-8">
            {links.map(link => (
              <Link 
                ...
                className={`text-lg uppercase tracking-[0.3em] font-bold ${location.pathname === link.path ? 'text-primary' : 'text-slate-400'}`}
              >
                {link.name}
              </Link>
            ))}
